...
- df.value_counts() -> データの中にあるデータの種類の個数を計算する。normalize=trueで確率に変換できる。そのあとに.sort_index()で昇順に並び替えることができる
- データ.index,データ.values -> データの要素が2つの場合は[.何とか]という形で表すことができる
- plt.xticks() -> 軸のメモリの間隔を調節する
- [何とか if 条件 else 何とか for 値 in 配列]で簡単に表すことができる
- plt.scatter(c = ) -> 色を指定することができる
- データを2つに分けてそれぞれ別の色で加えて合体させれば複数のデータをあわせることができる。必ずしも一回でplt.scatter()などしなくていい
### ABCを解いてての気づき
- string -> str
- !ok -> not ok
- a && b -> a and b
